namespace Corelib.Migrations.IdentityDataContext
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<Corelib.IdentityDataContext>
    {
        ...
        protected override void Seed(Corelib.IdentityDataContext context)
        {
            ...
            AddRole(context, "CanViewOtherInformations");
            ...
            AddRole(context, "CanViewUserManagement");
            ...
            context.Relationships.AddOrUpdate(t => t.Code, new Relationship[]
            {
                ...
                new Relationship(){ Code = "DEP0000164", Description = "ADOPTED DAUGHTER", Gender = 2},
        ...
        private void GenerateTriggers(Corelib.IdentityDataContext context)
        {
            ...
            sql = @"IF OBJECT_ID('TrGrOnDelete_Groups') IS NOT NULL DROP TRIGGER TrGrOnDelete_Groups";
            context.Database.ExecuteSqlCommand(sql);
            sql = @"
                CREATE TRIGGER [dbo].[TrGrOnDelete_Groups]
                   ON  [dbo].[Groups]
                   AFTER DELETE
                AS 
                BEGIN
                      ...
                      DELETE T1
                      FROM
                          GroupApplicationUsers T1
						  INNER JOIN DELETED T2 ON T1.Group_Id = T2.Id
                          INNER JOIN ApplicationRoleGroups T3 ON T2.Id = T3.Group_Id
                          INNER JOIN AccessibleGroups T4 ON T3.Group_Id = T4.Group_Id
					  WHERE
                           NOT EXISTS (
                              SELECT
                                 *
                              FROM
                                 Groups T101                                 
								 INNER JOIN ApplicationRoleGroups T102 ON T101.Id = T102.Group_Id
								 INNER JOIN AccessibleGroups T103 ON T102.Group_Id = T103.Group_Id
                                 WHERE
                                    T101.Id = T1.Group_Id
                          )
