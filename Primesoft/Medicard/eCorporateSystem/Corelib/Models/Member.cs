    ...
    public class Member : BaseModel, IValidatableObject
    {
        ...
        public string AppliedPlanDescription
        {
            get
            {
                ...
                {
                    var roomRate = legacyDb.LegacyRoomRates
                        .Include(t => t.LegacyPlan)
                        .Include(t => t.LegacyPaymode)
                        .FirstOrDefault(t => t.Id == this.AppliedPlan) ?? new LegacyRoomRate() { LegacyPlan = new LegacyPlan() };
            ...
            get
            {
                using (var legacyDb = new LegacyDataContext())
                {
                    var roomRate = legacyDb.LegacyRoomRates
                        .Include(t => t.LegacyPlan)
                        .Include(t => t.LegacyPaymode)
                        .FirstOrDefault(t => t.Id == this.OptionalPlan.Value)
                        ?? new LegacyRoomRate() { LegacyPlan = new LegacyPlan() };
                    return roomRate.LongDescription;
                }
            }
        }
        ...
        [Display(Name = "Number of allowed Dependents")]
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            using (var db = new IdentityDataContext())
            {
                ...
                {
                    if (this.Id == 0 && db.Members.Any(t => t.Status != MembershipStatus.Cancelled && t.LastName.ToLower() == this.LastName.ToLower() && t.FirstName.ToLower() == this.FirstName.ToLower() && t.DateOfBirth == this.DateOfBirth && t.AccountCode == this.AccountCode && t.EndorsementBatchId != this.EndorsementBatchId &&
                    ...
                    else if (this.Id == 0 && LegacyHelper.IsLegacyActivePrincipal(null, this.AccountCode, this.Code, this.FirstName, this.MiddleName, this.LastName, this.DateOfBirth))
                    {
            ...
            if (string.IsNullOrEmpty(this.Area) && LegacyHelper.GetLegacyArea(this.AccountCode).ToList().Count() > 0)
            {
